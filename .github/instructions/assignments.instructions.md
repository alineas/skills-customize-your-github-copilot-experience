...
- `# 📘 Atividade: [Título da Atividade]` — Substitua `[Título da Atividade]` por um nome curto e descritivo (ex: `Fundamentos de Python`, `Loops e Condicionais`, `Funções e Módulos`).
...
- `## 📝 Tarefas` — Para cada tarefa, use `### 🛠️ [Título da Tarefa]`:
   ...
   - Na seção `#### Descrição`, indique claramente o que o aluno deve fazer.
   - Na seção `#### Requisitos`, mantenha a frase `O programa concluído deve:` antes da lista.
   - Nos requisitos, use bullet points para listar resultados ou funcionalidades esperadas. Seja específico e mensurável.
